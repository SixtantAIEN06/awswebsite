...
def selected(request):
    ...
    #url=f'https://westus.api.cognitive.microsoft.com/luis/v2.0/apps/47fdbcf4-00cb-4e9a-a55c-df61cef1a102?verbose=true&timezoneOffset=0&subscription-key=06ee453f293849388126bae2d941d69f&q={luis}')
    url=f'https://westus.api.cognitive.microsoft.com/luis/v2.0/apps/0f4a4850-1d19-4e98-b4f1-73d12f6dff5f?verbose=true&timezoneOffset=0&subscription-key=734777fca0ee4ff7b70de96cd5ca8dfd&q={luis}')
...
def upload(request):
    ...
    if request.method =='POST' and request.FILES['photoupload']:
        myfile=request.FILES['photoupload']
        fs = FileSystemStorage(location='home/static/images/')
        myfile.name=''.join(random.choice(string.ascii_letters + string.digits) for x in range(10))+".jpg"
        fs.save(myfile.name,myfile)
